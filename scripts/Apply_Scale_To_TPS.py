...
def read_scale_csv(path: str, specimen_col: str, scale_col: str, invert: bool = False) -> Dict[str, str]:
    ...
    #invert to 1/scale if requested (e.g., if CSV has pix/mm but we want mm/pix)
    if invert:
        for k in mapping:
            try:
                val = float(mapping[k])
                if val != 0:
                    mapping[k] = str(1.0 / val)
                else:
                    print(f'Warning: zero scale value for {k}, cannot invert.', file=sys.stderr)
            except ValueError:
                print(f'Warning: non-numeric scale value for {k}: {mapping[k]}, cannot invert.', file=sys.stderr)
...
def process_tps(input_path: str, output_path: str, mapping: Dict[str, str]) -> None:
    ...
    for idx, block in enumerate(blocks):
        ...
        if scale is not None:
            ...
            unmatched_specimens.append(image_or_id if image_or_id else f"<no IMAGE/ID in block {idx + 1}>")
    ...
    if unmatched_blocks:
        print('Unmatched block indices:', unmatched_blocks)
        print("Unmatched specimen names/IDs:")
        for name in unmatched_specimens:
            print("  ", name)
...
def main(argv=None):
    ...
    p.add_argument('--scale-col', default='scale', help='CSV scale column name')
    p.add_argument('--invert-scale', action='store_true', help='Invert the scale values? (e.g., if CSV has pix/mm)')
    args = p.parse_args(argv)

    mapping = read_scale_csv(args.csv, args.specimen_col, args.scale_col, invert=args.invert_scale)
    if not mapping:
